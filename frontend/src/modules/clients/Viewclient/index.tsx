    ...
    const columns: GridColDef[] = [
        ...
        },
        {
            ...
            renderCell: (params: GridRenderCellParams<Date>) => (
                <>  <Link href={`/clients/${router.query.id}/${params.id}`} className={styles.link} style={{ marginRight: '10px' }}>
                        <Icon icon="ic:baseline-remove-red-eye" />    View
                    </Link>
                    <Link href={`/clients/${router.query.id}/${params.id}`} className={styles.link} style={{ marginRight: '10px' }}>
                        <Icon icon="ph:trash-bold" onClick={() => { console.log('deleted') }} /> Delete
                    </Link>
                    <Link href={`/clients/${router.query.id}/${params.id}`} className={styles.link}><Icon icon="uil:pen" />Edit</Link></>
              ),
            width: 300
          },
      ];
