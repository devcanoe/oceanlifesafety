...
const columns: GridColDef[] = [
    ...
    },
    {
        ...
        renderCell: (params: GridRenderCellParams<Date>) => (
            <>  <Link href={`/clients/${params.id}`} className={styles.link} style={{ marginRight: '10px' }}>
                    <Icon icon="ic:baseline-remove-red-eye" />    View
                </Link>
                <Link href={`/clients/${params.id}`} className={styles.link} style={{ marginRight: '10px' }}>
                <Icon icon="ph:trash-bold" onClick={() => { console.log('deleted') }} /> Delete
                </Link>
                <Link href={`/clients/${params.id}`} className={styles.link}><Icon icon="uil:pen" />Edit</Link></>
          ),
        width: 300
      },
  ];
        ...
        <>
            <main className={styles.container}>
                ...
                <Navbar title={'Clients'}
                    ...
                    buttonLabel={'Add Client'} buttonAction={function (): void {
...
export function Navbar({ rowSelectionModel, title, buttonLabel, buttonAction, deleteAction, customButtonLabel, customButtonAction }: INavbar) {
    return (
        <>
            <nav className={styles.clientnavbar}>
                ...
                <div className={styles.clientbutton}>
                    { rowSelectionModel?.length || 0 > 0 &&
                        <Icon icon="ph:trash-bold" color="red" width={30} height={30} onClick={deleteAction} />
                    }
