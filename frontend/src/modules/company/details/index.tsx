...
export default function CompanyDetail(props: ICompanyDetail) {
  ...
  const {
    ...
    isLoading,
    isSuccess,
  } = useGetOneCompanyQuery({ id: props.id });
    ...
    <>
      ...
      <div className={styles.container}>
        <header className={styles.header}>
          <div>{isSuccess && company?.data.name}</div>
          <div className={styles.buttons}>
            <Button
              icon={<Icon icon="ic:baseline-plus" />}
              label="Ship"
              onClick={shipModalToggleHandler}
            />
            <Button
              icon={<Icon icon="ic:baseline-plus" />}
              label="Raft"
              onClick={raftModalToggleHandler}
            />
            <Button
              icon={<Icon icon="ic:baseline-plus" />}
              label="form"
              onClick={formModalToggleHandler}
            />
          </div>
        </header>
